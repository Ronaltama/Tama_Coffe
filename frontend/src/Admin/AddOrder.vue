...
const fetchProducts = async () => {
  try {
    ...
    if (response.data && response.data.success) {
      // Normalisasi product fields: pastikan ada category_name dan imageUrl
      products.value = response.data.data.map((product) => {
        // support various API shapes: product.category_name OR product.category.name OR product.category?.name
        const categoryName =
      ...
      console.warn("fetchProducts: unexpected response", response.data);
    }
  } catch (error) {
...
const fetchCategories = async () => {
  try {
    ...
    if (response.data && response.data.success) {
      // response.data.data bisa berisi array objek { id, name } atau array string
      const raw = response.data.data || [];
      const names = raw.map((c) =>
        typeof c === "string" ? c : c?.name ?? String(c?.id ?? "")
      );

      // unikkan dan pastikan ada "All"
      const uniq = Array.from(new Set(names));
      categories.value = ["All", ...uniq];
      console.log("Categories loaded:", categories.value);
    } else {
      console.warn("fetchCategories: unexpected response", response.data);
    }
  } catch (error) {
...
const isDrink = (categoryName) => {
  ...
  // Lebih toleran: jika mengandung kata 'drink', 'beverage', atau 'minuman'
  return (
...
<template>
  <div class="p-6 space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div class="grid grid-cols-2 gap-4 max-w-md">
        <button @click="orderType = 'dine-in'" :class="[
    ...
    <div v-if="orderType === 'dine-in'" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div class="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-3">
        <button v-for="table in tables" :key="table.id" @click="selectedTable = table.id" :class="[
    ...
    <div v-if="orderType" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl">
        ...
        <div>
          ...
          <input v-model="customerPhone" type="tel" placeholder="08xxxxxxxxxx"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-amber-500 focus:border-amber-500" />
        </div>
        <div class="md:col-span-2">
          <label class="block text-sm font-medium text-gray-700 mb-2">Catatan (Opsional)</label>
          <textarea v-model="orderNote" rows="2" placeholder="Tambahkan catatan untuk order ini..."
    ...
    <div v-if="orderType" class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="relative mb-4">
          ...
          <input type="text" v-model="searchTerm" placeholder="Cari menu..."
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-[600px] overflow-y-auto">
          <div v-for="product in filteredProducts" :key="product.id"
            ...
            <button @click="handleAddToCart(product)" :disabled="product.status === 'unavailable'"
      ...
      <div class="w-full lg:w-96">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-6">
          ...
          <div class="p-5 space-y-3 max-h-[400px] overflow-y-auto">
            ...
            <div v-for="(item, index) in cart" :key="`${item.id}-${item.variant || 'no-variant'}`"
          ...
          <div class="p-5 border-t border-gray-200 space-y-3">
            ...
            <button @click="handlePayment" :disabled="!canProceedToPayment"
    ...
    <div v-if="showVariantModal"
      ...
      <div class="bg-white rounded-xl p-6 max-w-md w-full mx-4 border border-gray-200 shadow-lg">
        <h3 class="text-xl font-bold text-gray-900 mb-4">Pilih Varian</h3>
        <p class="text-gray-600 mb-4">{{ selectedProduct?.name }}</p>

        <div class="space-y-2 mb-6">
          <label class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer hover:bg-gray-50" :class="selectedVariant === 'Ice'
            ? 'border-amber-500 bg-amber-50'
            : 'border-gray-200'
            ">
          ...
          <label class="flex items-center gap-3 p-3 border-2 rounded-lg cursor-pointer hover:bg-gray-50" :class="selectedVariant === 'Hot'
            ? 'border-amber-500 bg-amber-50'
            : 'border-gray-200'
            ">
  ...
  <!-- Modal: Pembayaran -->
  <div v-if="showPaymentModal" class="fixed inset-0 flex items-center justify-center z-50 bg-white/20 backdrop-blur-md"
    ...
    <div class="bg-white rounded-xl p-6 max-w-md w-full mx-4 border border-gray-200 shadow-lg">
      ...
      <div v-if="paymentMethod === 'cash'" class="mb-4">
        ...
        <div class="grid grid-cols-3 gap-2 mt-2">
          <button v-for="amount in [50000, 100000, 150000]" :key="amount" @click="quickAmount(amount)"
