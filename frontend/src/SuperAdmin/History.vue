...
const fetchOrders = async () => {
  try {
    ...
    // Panggil endpoint history khusus superadmin
    const res = await axios.get(
      `${API_BASE}/superadmin/orders/history`,
      config
    );
    ...
    if (err.response) {
      if (err.response.status === 403) {
        error.value =
          "Akses ditolak. Pastikan Anda login dengan akun yang memiliki hak akses.";
...
const viewOrderDetails = (id) => {
  router.push({ name: 'SuperAdminOrderDetail', params: { id } });
};
