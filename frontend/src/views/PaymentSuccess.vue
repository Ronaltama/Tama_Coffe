  ...
  <div class="bg-white min-h-screen">
    <div class="max-w-md mx-auto bg-white min-h-screen font-sans flex flex-col">
      ...
      >
        <button
          @click="closeModal"
          class="p-1 -ml-2 text-gray-800 hover:text-gray-600 transition-colors"
        >
          <svg
      ...
      <main class="flex-1 px-6 py-8 overflow-y-auto" v-if="hasData">
        ...
        <section class="text-center mb-8">
          <div
            class="inline-block bg-neutral-50 rounded-xl px-8 py-4 border border-neutral-200"
          >
            <p class="text-3xl font-bold text-gray-900 mb-1 tracking-wider">
              {{ displayNumber }}
            </p>
        ...
        <section
          class="mb-6 bg-white border border-gray-100 rounded-xl shadow-sm p-4"
        >
          <h3
      ...
      >
        ...
        <p class="text-gray-500 text-sm">
          Sesi pesanan Anda telah berakhir. Silakan buat pesanan baru.
        </p>
      </main>
...
onMounted(() => {
  ...
  if (finishedData) {
    ...
    orderType.value = orderData.orderType || "Dine In";
    isReservation.value = orderType.value === "Reservasi";
    ...
    if (isReservation.value) {
      // Untuk reservasi, tampilkan booking code (prioritas) atau reservation ID sebagai fallback
      displayNumber.value =
        orderData.reservationMeta?.bookingCode ||
        orderData.reservationMeta?.reservationId ||
        "RES-" + Math.floor(Math.random() * 10000);

      // Tampilkan tanggal reservasi
      if (orderData.reservationDate && orderData.reservationTime) {
        const date = new Date(orderData.reservationDate).toLocaleDateString(
          "id-ID",
          {
            day: "2-digit",
            month: "short",
        ...
        dateTime.value = `${date} / ${orderData.reservationTime} WIB`;
      } else if (orderData.reservationMeta) {
        dateTime.value = `${orderData.reservationMeta.date} / ${orderData.reservationMeta.time}`;
      } else {
        dateTime.value = "-";
      }
    } else {
      // Untuk order biasa, tampilkan nomor meja
      displayNumber.value = orderData.table || "-";

      const now = orderData.transactionTime
        ? new Date(orderData.transactionTime)
        : new Date();
...
const backToMenu = () => {
  ...
  // Clear session data setelah success
  if (orderType.value !== "Reservasi") {
    // Untuk order biasa, clear semua data
    localStorage.removeItem("currentTableId");
    localStorage.removeItem("currentTableNumber");
    localStorage.removeItem("orderType");
  } else {
    // Untuk reservasi, keep reservation details tapi clear cart data
    localStorage.removeItem("cart");
    localStorage.removeItem("cartNotes");
  }

  // Redirect berdasarkan order type
  if (isReservation.value) {
    router.push("/reservation");
  } else {
    const tableId = props.tableId || localStorage.getItem("currentTableId");
    if (tableId) {
      router.push(`/order/${tableId}/menu`);
    } else {
