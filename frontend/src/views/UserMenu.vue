<template>
  <div class="bg-neutral-100 min-h-screen">
    <div class="max-w-md mx-auto bg-white min-h-screen font-sans relative shadow-2xl">
      ...
      >
        <div class="bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.15)] rounded-t-2xl overflow-hidden border-t border-gray-100">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="relative p-2 bg-orange-50 rounded-xl text-orange-600">
                ...
                <span class="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full shadow-sm border border-white">
                  {{ totalItems }}
                </span>
              </div>
              
              <div class="flex flex-col">
                <span class="text-xs text-gray-500 font-medium">Total Estimasi</span>
                <span class="font-bold text-lg text-gray-900 leading-tight">Rp{{ totalPrice }}</span>
              </div>
            </div>

            <div class="flex items-center gap-2 text-gray-400">
              <span class="text-xs font-medium text-gray-500">{{ isCartOpen ? 'Tutup' : 'Lihat Detail' }}</span>
              <div class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center transition-transform duration-300" :class="{'rotate-180': isCartOpen}">
          ...
          >
            <div v-if="isCartOpen" class="bg-gray-50 border-t border-gray-100">
              <div class="overflow-y-auto custom-scrollbar p-4 space-y-3" style="max-height: 50vh;">
                <div 
                ...
                >
                  <div class="flex items-center gap-3 flex-1 min-w-0">
                    <div class="flex items-center bg-gray-100 rounded-lg p-1 border border-gray-200">
                      <button @click.stop="decrementCart(item.id)" class="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm text-gray-600 hover:text-red-600 active:scale-90 transition-transform">
                        <span class="text-xs font-bold leading-none">-</span>
                      </button>
                      <span class="text-xs font-bold w-6 text-center text-gray-700">{{ item.quantity }}</span>
                      <button @click.stop="incrementCart(item.id)" class="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm text-gray-600 hover:text-green-600 active:scale-90 transition-transform">
                        <span class="text-xs font-bold leading-none">+</span>
                      </button>
                    </div>
                    ...
                    <div class="flex flex-col overflow-hidden">
                      ...
                      <span v-if="item.variant !== 'food'" class="text-[10px] text-gray-500 uppercase tracking-wider">{{ item.variant }}</span>
                    </div>
                  </div>
...
onMounted(async () => {
  ...
  if (storedOrderType) orderType.value = storedOrderType;
